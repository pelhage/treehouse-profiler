...
xhr.onreadystatechange = function () {
    ...
    if (xhr.readyState === 4) {
        ...
        //GET & SORT LIST OF SKILL POINTS EARNED
        var pointsList = [];
        ...
        // Sort the colors to be placed in chart
        var chartColors = [];
        ...
        var profileHTML = '<h1>' + data.name + '</h1><span> : '+ pointsList[0][1].toLocaleString() + ' Total Points, '+ data.badges.length + ' Achievements</span>';
        var badgeHTML = '<ul class="badges">';

        for (var i = 0, j = data.badges.length - 1; i < j; j--) {
            badgeHTML += '<li class="badge">';
            badgeHTML += '<p class="badge--name">' + data.badges[j].name + '</p>';
        ...
        // document.getElementById('profile-image').innerHTML = '<img src="' + data.gravatar_url + '">';
